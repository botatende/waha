  ...
  await window.require('WAWebCmd').Cmd.refreshQR();
    ...
    _registerFramenavigatedHandler() {
        ...
        this.pupPage.on('framenavigated', async (frame) => {
            ...
            // During an unpaired/restore flow WhatsApp can emit logout_from_bridge
            // while replacing the previous registration. It is a navigation reset,
            // not the logout of an authenticated client. Treating it as terminal
            // deletes the profile being paired and makes the session fail after scan.
            if (isLogout && !this.hasAuthenticated) {
                this.lastLoggedOut = false;
            } else if (isLogout) {
                this.hasAuthenticated = false;
    ...
    async attachEventListeners() {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                await this.waitForStableExecutionContext();
                await this.attachEventListenersOnce();
                return;
            } catch (error) {
                lastError = error;
                ...
                if (!navigationRace || attempt === maxAttempts || !this.pupPage || this.pupPage.isClosed()) {
                    throw error;
                }

                this.emit('debug', `Listener injection interrupted by navigation; retry ${attempt}/${maxAttempts}`);
    ...
    async waitForStableExecutionContext() {
        await this.pupPage.waitForFunction(
            () => document.readyState === 'complete',
            { timeout: 30000 },
        );
        await this.pupPage.evaluate(() => document.location.href);
        await new Promise((resolve) => setTimeout(resolve, 500));
        await this.pupPage.evaluate(() => document.location.href);
    }

    async attachEventListenersOnce() {
        await exposeFunctionIfAbsent(
            this.pupPage,
            'onAddMessageEvent',
            (msg) => {
